...
import { applyBottomTabOrder, initBottomPanel } from '../bottomPanel.js';
...
describe('bottomPanel — initBottomPanel()', () => {
  ...
  it('restores a stored tab order without moving the close button slot', () => {
    const testDoc = buildDom();
    const tabBar = testDoc.querySelector('.bottom-panel-tabs');

    applyBottomTabOrder(tabBar, ['rag', 'chat', 'model']);

    const orderedTabs = Array.from(
      testDoc.querySelectorAll('.bottom-panel-tabs .bottom-tab')
    ).map((tab) => tab.dataset.tab);

    expect(orderedTabs.slice(0, 3)).toEqual(['rag', 'chat', 'model']);
  });

  // Validates: Requirements 5.4
  it('Property 6: exactly one tab and one content pane are active at a time', { timeout: 10000 }, () => {
    fc.assert(
      fc.property(
